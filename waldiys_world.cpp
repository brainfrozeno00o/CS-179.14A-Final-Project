...
struct Cannon {
	...
	bool isLookingLeft; //default is that it looks to the left

	Cannon(float px, float py, bool orientation){
		unitCannon.setSize(Vector2f(CANNON_W, CANNON_H));
		unitCannon.setPosition(px, py);
		isLookingLeft = orientation; //if false it is looking to the right, true otherwise
		if(!isLookingLeft) unitCannon.setScale(-1,1); //mirror so that it will face to the right
	}
};

struct Bullet {
	CircleShape unitBullet;
	bool isGoingLeft; //determine direction of where the bullet is going, default direction is going left
...
float timeBeforePause, numSec; //timeBeforePause for pausing, numSec for how many seconds each bullet will fire

bool isInsideWindow(Vector2f pos, Vector2f size){ //for updating the uniform grid, and for shooting bullets
	if ((pos.x + size.x) < 0 || pos.x > WINDOW_W || (pos.x + size.x) < 0 || pos.y > WINDOW_H) return false; 
	return true;
}
...
void checkWorldCollision(){
	...
	//check for collision between objects if they are inside the same grid
	...
	//player-tile collision
		...
		if(!(**g).bullets.empty() && !(**g).tiles.empty()){
			for(vector<Bullet*>::iterator bu = (**g).bullets.begin(); bu != (**g).bullets.end() && gameState == 1; ++bu){
				for(vector<RectangleShape*>::iterator ti = (**g).tiles.begin(); ti != (**g).tiles.end() && gameState == 1; ++ti){
					if((**bu).unitBullet.getGlobalBounds().intersects((**ti).getGlobalBounds())) (**bu).unitBullet.setPosition(900,900); //they are colliding in some way
				}
			}
		}
	}
...
bool cannonShoot(float secondsElapsed){
	if (secondsElapsed > numSec){ //each bullet (is) constantly spawns/(fired) every 1.8 seconds, thus when it reaches this point, it will load another bullet
		Bullet* shootMe; 
		timeBeforePause = 0;
		for(list<Cannon*>::iterator ca = cannons.begin(); ca != cannons.end(); ++ca){
			...
			for(list<Bullet*>::iterator bu = bullets.begin(); bu != bullets.end() && !found; ++bu){
				if (!isInsideWindow((**bu).unitBullet.getPosition(), Vector2f((**bu).unitBullet.getRadius(), (**bu).unitBullet.getRadius()))) { //loading the bullet to the screen
					...
					if ((**ca).isLookingLeft) shootMe->isGoingLeft = true;
					else shootMe->isGoingLeft = false;
					newPos += Vector2f(0, BULLET_SIZE + 6.25f); //position to shoot roughly from the middle of the cannon
...
void initializeGame(){ 
	...
	//CREATE CONSTANT TIME
	cin >> numSec;
	//initializing the grid and setting the game state to 0
	initializeGrid();
...
int main(){
	...
	window.create(VideoMode(WINDOW_W, WINDOW_H), "Waldiy's World", Style::Default, settings);
	window.setFramerateLimit(FPS);
	window.setKeyRepeatEnabled(true); //weird... it can work even if it's false
	...
	while(window.isOpen()){
		...
		if (gameState == 0) window.draw(titleScreen);
		else {
